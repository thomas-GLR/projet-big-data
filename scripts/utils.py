...
from sklearn.metrics import make_scorer, roc_auc_score, precision_score, recall_score, mean_absolute_error, r2_score, \
    mean_squared_error, accuracy_score, f1_score, balanced_accuracy_score, classification_report
...
def create_ref_data(df: pd.DataFrame, output_filepath: str):
    """
    ...
    df, label_encoder, target_encoder = encode_categorical_columns(df, target_type="multiclass")

    answer_dass_to_keep = ['Q10A', 'Q38A', 'Q13A', 'Q34A', 'Q16A', 'Q3A', 'Q37A', 'Q26A']

    df = filter_dass_answers_by_giving_columns(df, answer_dass_to_keep)

    X, y, features_label = split_features_target(df)

    scaler = StandardScaler()
    scaler.fit(X)

    ref_df = pd.DataFrame(X, columns=features_label)
    ref_df["target"] = y
    ...
    save_artifact(target_encoder, "target_encoder.pkl")
    save_artifact(scaler, "scaler.pkl")

    return ref_df, label_encoder, target_encoder, scaler
...
def train_model(X: np.ndarray, y: np.ndarray, save: bool = True):
    ...
    model = LogisticRegression(C=0.1, max_iter=500, penalty='l1', solver='saga')
    ...
    print(f"Model trained - Accuracy: {metrics['accuracy']:.4f}, Precision: {metrics['precision']:.4f}, Recall: {metrics['recall']:.4f}, F1 Score: {metrics['f1_score']:.4f}, Balanced Accuracy: {metrics['balanced_accuracy']:.4f}")
...
def retrain_model():
    ...
    X = combined_df.drop(columns=["target", "user_feedback", "prediction"]).values
...
def transform_single_input(data: dict, label_encoder: LabelEncoder, target_encoder: TargetEncoder):
    """
    ...
    df, label_encoder, target_encoder = encode_categorical_columns(df, target_type="multiclass", label_encoder=label_encoder, target_encoder=target_encoder)
...
def clean_dataset_and_save(file_name = 'dataset.csv', sep = ',') -> pd.DataFrame:
    ...
    df = load_raw_data_without_useless_columns(data_path)
...
def encode_categorical_columns(df: pd.DataFrame, target_type: str, label_encoder: LabelEncoder = None, target_encoder: TargetEncoder = None, target_encoder_columns=None, binary_columns=None) -> tuple[pd.DataFrame, LabelEncoder, TargetEncoder]:
    """
    ...
    unique_labels = np.array([0, 1, 2, 3, 4])

    if target_encoder is None:
        target_encoder = TargetEncoder(target_type=target_type, smooth="auto")
        Y = get_target(df)
        target_encoder.fit(df[target_encoder_columns], Y)

    encoded = target_encoder.transform(df[target_encoder_columns])
    ...
    return df, label_encoder, target_encoder
